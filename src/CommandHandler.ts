...
export default class CommandHandler {
  ...
  private async setUp(
    ...
    if (dir) {
      ...
      console.log(
        `SpaceCommands > Loaded ${amount} command${amount === 1 ? '' : 's'}.`
      )
  ...
  public async registerCommand(
    ...
    if (missing.length && instance.showWarns) {
      console.warn(
        `SpaceCommands > Command "${names[0]}" does not have the following properties: ${missing}.`
      )
    }

    if (testOnly && !instance.testServers.length) {
      console.warn(
        `SpaceCommands > Command "${names[0]}" has "testOnly" set to true, but no test servers are defined.`
      )
    }

    if (slash !== undefined && typeof slash !== 'boolean' && slash !== 'both') {
      throw new Error(
        `SpaceCommands > Command "${names[0]}" has a "slash" property that is not boolean "true" or string "both".`
      )
    }
    ...
    if (slash && !(builtIn && !instance.isDBConnected())) {
      if (!description) {
        throw new Error(
          `SpaceCommands > A description is required for command "${names[0]}" because it is a slash command.`
        )
      }

      if (minArgs !== undefined && !expectedArgs) {
        throw new Error(
          `SpaceCommands > Command "${names[0]}" has "minArgs" property defined without "expectedArgs" property as a slash command.`
        )
      }
      ...
      if (options.length) {
        for (const key in options) {
          ...
          if (name !== lowerCase && instance.showWarns) {
            console.log(
              `SpaceCommands > Command "${names[0]}" has an option of "${name}". All option names must be lower case for slash commands. SpaceCommands will modify this for you.`
            )
          }
          ...
          if (lowerCase.match(/\s/g)) {
            ...
            console.log(
              `SpaceCommands > Command "${names[0]}" has an option of "${name}" with a white space in it. It is a best practice for option names to only be one word. SpaceCommands will modify this for you.`
            )
          }
