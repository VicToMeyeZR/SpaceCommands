...
export default class SpaceCommands extends EventEmitter {
  ...
  private _componentHandler: ComponentHandler | null = null
  ...
  private async setUp(client: Client, options?: Options) {
    ...
    if (
      ...
      throw new Error(
        "SpaceCommands > The 'commands' directory must be an absolute path. This can be done by using the 'path' module."
      )
    }
    ...
    if (
      ...
      throw new Error(
        "SpaceCommands > The 'features' directory must be an absolute path. This can be done by using the 'path' module."
      )
    }
    ...
    this._modalHandler = new ModalHandler(
      this,
      modalsDir,
      typeScript
    )
  ...
  public setMongoPath(mongoPath: string | undefined): SpaceCommands {
    console.warn(
      'SpaceCommands > .setMongoPath() no longer works as expected. Please pass in your mongo URI as a "mongoUri" property using the options object.'
    )
    return this
  }
  ...
  public setDefaultPrefix(defaultPrefix: string): SpaceCommands {
  ...
  public setPrefix(guild: Guild | null, prefix: string): SpaceCommands {
    if (guild) {
  ...
  public setColor(color: ColorResolvable | null): SpaceCommands {
    this._color = color
    return this
  }
  ...
  public setCategorySettings(category: ICategorySetting[]): SpaceCommands {
    for (let { emoji, name, hidden, customEmoji } of category) {
      ...
      if (this.isEmojiUsed(targetEmoji)) {
        console.warn(
          `SpaceCommands > The emoji "${targetEmoji}" for category "${name}" is already used.`
        )
      }
  ...
  public setBotOwner(botOwner: string | string[]): SpaceCommands {
    console.log(
      'SpaceCommands > setBotOwner() is deprecated. Please specify your bot owners in the object constructor instead.'
    )
  ...
  public setDefaultLanguage(defaultLanguage: string): SpaceCommands {
