...
class FeatureHandler {
  ...
  private setup = async (dir: string, typeScript: boolean) => {
    ...
    if (amount > 0) {
      console.log(
        `SpaceCommands > Loading ${amount} listener${amount === 1 ? '' : 's'}...`
      )
      ...
      console.log(
        `SpaceCommands > Loaded ${amount} listener${amount === 1 ? '' : 's'}.`
      )
    }
  }
  ...
  private registerFeature = (file: any, fileName: string) => {
    ...
    if (config) {
      ...
      if (missing.length && this._instance.showWarns) {
        console.warn(
          `SpaceCommands > Feature "${fileName}" has a config file that doesn't contain the following properties: ${missing}`
        )
      }
    } else if (this._instance.showWarns) {
      console.warn(
        `SpaceCommands > Feature "${fileName}" does not export a config object.`
      )
    }
    ...
    if (config && config.loadDBFirst === true) {
      console.warn(
        `SpaceCommands > config.loadDBFirst in features is no longer required. MongoDB is now connected to before any features or commands are loaded.`
      )
    }
