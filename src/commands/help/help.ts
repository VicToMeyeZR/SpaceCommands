...
const sendHelpMenu = (message: Message, instance: SpaceCommands) => {
...
module.exports = {
  ...
  init: (client: Client, instance: SpaceCommands) => {
  ...
  callback: (options: ICallbackObject) => {
    ...
    if (guild && !guild.me?.permissions.has('SEND_MESSAGES')) {
      console.warn(
        `SpaceCommands > Could not send message due to no permissions in channel for ${guild.name}`
      )
      return
    }
